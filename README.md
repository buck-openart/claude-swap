...
- **Strategies** (`--strategy`, or `cswap config set autoswitch.strategy`): `best` (default) stays put until the active account nears its limit, then moves to the account with the most quota left. `consume-first` proactively keeps you on the account whose **weekly window resets soonest** — use-it-or-lose-it — switching to a sooner-resetting account (with room to spare) even below the threshold, so perishable weekly quota isn't wasted. `priority` follows a rank-ordered list you set instead of chasing headroom: `--priority-accounts <num|email|alias>,...` (or `cswap config set autoswitch.priorityAccounts enterprise,max`). It recalls to the highest-ranked account that's below the threshold *even while the active, lower-ranked account is itself still healthy* — the opposite of the no-return bar's usual "don't flap back" rule. Recall is scoped to exactly that case: once the active account is at its limit, the ordinary headroom ranking and `--fallback-account` take over, so a rank order can never strand you on an exhausted account. Every entry is capped by the threshold, the last one included — recall departs a healthy account, so landing on a spent one would only bounce straight back. An account not in the list ranks below every account that is; a quarantined entry is skipped in favor of the next one down; recall respects `--cooldown-seconds`. An identifier that can't be recalled to, or that duplicates one ranked higher, is dropped with a one-time warning rather than breaking the rest of the list, and `--strategy priority` with an empty list warns that it behaves exactly like `best`.
